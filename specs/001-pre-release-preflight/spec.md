...
- **FR-001d**: When `lint:shell`, `typecheck`, or `test` scripts are present in package.json, `preflight` MUST invoke each of them in this order: `lint:shell`, `typecheck`, `test`. If any of these scripts fail, `preflight` MUST fail immediately and skip remaining optional scripts. When any of these scripts are absent, their corresponding steps MUST be skipped and MUST NOT cause the preflight to fail.
