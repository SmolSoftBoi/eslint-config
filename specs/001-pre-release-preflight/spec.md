...
- **FR-001d-i**: When `lint:shell`, `typecheck`, or `test` scripts are present in package.json, `preflight` MUST invoke the mandatory `lint` script first (per **FR-001a**), then invoke each present optional script in this order: `lint:shell`, `typecheck`, `test`.
