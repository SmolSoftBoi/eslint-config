...
- **FR-001d-i**: `preflight` MUST run the mandatory `lint` script before any optional scripts are executed.
- **FR-001d-ii**: After running the mandatory `lint` script (see **FR-001a**), `preflight` MUST sequentially invoke the following optional scripts in this order: `lint:shell`, `typecheck`, `test`. Each optional script MUST be invoked only if it is present in the `scripts` field of `package.json`; any script that is not defined MUST be skipped. Failure behavior is governed by **FR-001d-iii**; skip behavior for absent scripts is governed by **FR-001d-iv**.
