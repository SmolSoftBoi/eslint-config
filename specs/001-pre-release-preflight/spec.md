...
- **FR-001d-ii**: After running the mandatory `lint` script (per **FR-001d-i**), `preflight` MUST then invoke any of these optional scripts that are present in package.json scripts, preserving the following relative order while skipping any that are missing: `lint:shell`, `typecheck`, `test`. This ordering is intentional: cheaper, lower-level checks (shell and typing) MUST run before more expensive or higher-level checks (tests) so that failures are reported as early as possible and unnecessary work is avoided.
...
- **FR-003c**: For local (non-CI) runs, tools MAY skip the packed tarball import step only when `SKIP_PACKED_IMPORT` is exactly `1` or `true`; for any other value (including `0`, `false`, or empty), the packed tarball import step MUST be executed.
