...
- **FR-001d-ii**: After running the mandatory `lint` script (per **FR-001d-i**), `preflight` MUST then invoke any of these optional scripts that are present in the `package.json` `scripts` field in this order, skipping any that are missing: `lint:shell`, `typecheck`, `test`.
