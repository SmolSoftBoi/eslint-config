...
- **FR-001d-ii**: After running the mandatory `lint` script (per **FR-001a**, and which MUST run first), `preflight` MUST then sequentially invoke any of these optional scripts that are present in the `package.json` `scripts` field, in the following order (with failure and skip behavior governed by **FR-001d-iii** and **FR-001d-iv**, and skipping any that are not defined): `lint:shell`, `typecheck`, `test`.
