...
- **FR-001d-ii**: After running the mandatory `lint` script, `preflight` MUST then sequentially invoke any of these optional scripts that are present in the `package.json` `scripts` field, in the following order (skipping any that are not defined): `lint:shell`, `typecheck`, `test`.
