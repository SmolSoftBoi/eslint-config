...
- **FR-001d-ii**: Consistent with **FR-001a**, after running the mandatory `lint` script (which MUST run first), `preflight` MUST then sequentially invoke any of these optional scripts that are present in the `package.json` `scripts` field, in the following order (skipping any that are not defined): `lint:shell`, `typecheck`, `test`.
